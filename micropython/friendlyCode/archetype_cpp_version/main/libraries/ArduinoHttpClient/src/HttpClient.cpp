                                ...
                                const char* aContentType, int aContentLength, const byte aBody[])
{
    ...
    Serial.println("\tNow we're connected, send the first part of the request");
    ...
    if (HTTP_SUCCESS == ret)
    {
        if (aContentType)
        {
            ...
            //Serial.println("\tSending content type");
        }

        if (aContentLength > 0)
        {
            sendHeader(HTTP_HEADER_CONTENT_LENGTH, aContentLength);
            //Serial.println("\tSending content length");
        }

        bool hasBody = (aBody && aContentLength > 0);
        //Serial.println("\tHas body: " + String(hasBody));
    ...
    //Serial.println("\tReturning ret: " + String(ret));
    return ret;
}
...
int HttpClient::responseStatusCode()
{
    if (iState < eRequestSent)
    {
        Serial.println("\tHTTP request not sent yet");
        return HTTP_ERROR_API;
    }

    Serial.println("\tWaiting for response status code...");
    //Serial.println("\tiState: " + String(iState)); //2
    //Serial.println("\tiStatusCode: " + String(iStatusCode)); //0
    ...
    do
    {
        ...
        iStatusCode = 0;
        iState = eRequestSent;
        Serial.println("\tReading response status code...");
               ...
               ( (millis() - timeoutStart) < iHttpResponseTimeout ))
        {
            if (available())
            {
                ...
                if (c != -1)
                {
                    ...
                    // Debugging print statements inside the loop
            ...
            else
            {
                ...
                //Serial.println("\tWe haven't got any data, so let's pause to allow some to arrive");
            }
        }
